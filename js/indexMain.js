...
// 获取所有的滑动窗口
let scrollables = document.querySelectorAll(".card.IndexMainShell");
// 为每个滑动窗口添加滚动监听
scrollables.forEach(function (scrollable) {
  scrollable.addEventListener("scroll", function () {
    // 当一个窗口被滚动时，重置其他窗口的滚动位置
    scrollables.forEach(function (otherScrollable) {
      if (otherScrollable !== scrollable) {
        otherScrollable.scrollTop = 0;
      }
    });
  });
});
